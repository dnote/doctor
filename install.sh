...
install_dnote() {
  ...
  if ! check_platform "$os" "$arch"; then
    ...
    print_error "Please compile manually from https://github.com/dnote/doctor"
    exit 1
  fi
...
exit_error() {
  if [ "$?" -ne 0 ]; then
    print_error "A problem occurred while installing Dnote. Please report it on https://github.com/dnote/doctor/issues so that we can help you."
  fi
}
